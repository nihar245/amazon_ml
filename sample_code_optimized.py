"""
Amazon ML Challenge 2025 - OPTIMIZED Model Inference
Uses DeBERTa-v3-small model trained with train_optimized.py
Target: SMAPE 30-40% on test set
"""
...
print("="*60)
print("Amazon ML Challenge 2025 - OPTIMIZED Model Inference")
print("="*60)
...
class TestDataset(Dataset):
    def __init__(self, df, tokenizer, brand_enc, category_enc, unit_enc, scaler, max_length=256):
        ...
        for idx in tqdm(range(len(df)), desc="Features"):
            features = extract_optimized_features(df.iloc[idx]['catalog_content'])
...
def main():
    ...
    try:
        ...
        with open('models/brand_encoder_optimized.pkl', 'rb') as f:
            brand_enc = pickle.load(f)
        with open('models/category_encoder_optimized.pkl', 'rb') as f:
            category_enc = pickle.load(f)
        with open('models/unit_encoder_optimized.pkl', 'rb') as f:
    ...
    try:
        ...
        checkpoint = torch.load('models/best_model_optimized.pth', map_location=device)
