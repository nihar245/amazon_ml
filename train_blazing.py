"""
Amazon ML Challenge 2025 - BLAZING FAST Training
EXTREME optimizations for maximum speed with good accuracy
Target: 2-3 hours on P100, 40-45% SMAPE
"""
...
def extract_enhanced_features(text):
    """Extract 20 most important features (balanced)"""
    features = {
...
class BlazingDataset(Dataset):
    def __init__(self, df, tokenizer, brand_enc, category_enc, unit_enc, 
        ...
        for idx in tqdm(range(len(df)), desc="Features"):
            feat = extract_enhanced_features(df.iloc[idx]['catalog_content'])
...
class BlazingPredictor(nn.Module):
    def __init__(self, text_model_name='distilroberta-base', numeric_dim=20, hidden_dim=384):
        ...
        # DistilRoBERTa: 82M params, much faster than DeBERTa
        self.text_encoder = AutoModel.from_pretrained(text_model_name)
        text_dim = self.text_encoder.config.hidden_size  # 768
        
        # Streamlined fusion (fewer layers = faster)
...
def train_blazing(model, train_loader, val_loader, epochs=10, lr=3e-4, grad_accum=8, resume_from=None):
    ...
    print(f"  - Model: DistilRoBERTa (82M params, 3x faster)")
    print(f"  - Sequence: 256 tokens (vs 384)")
    print(f"  - Features: 20 (balanced)")
    ...
    print(f"  - Learning rate: {lr} (faster convergence)")
    print(f"  - Expected: 2-3 hours on P100, 40-45% SMAPE\n")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        torch.save(checkpoint, 'models/checkpoint_blazing_latest.pth', _use_new_zipfile_serialization=False)
        
        # Save epoch-specific checkpoint
        torch.save(checkpoint, f'models/checkpoint_blazing_epoch_{epoch+1}.pth', _use_new_zipfile_serialization=False)
        print(f'💾 Checkpoint saved: epoch {epoch+1}')
        
        # Save best model
        if val_smape < best_val_smape:
            best_val_smape = val_smape
            torch.save(checkpoint, 'models/best_model_blazing.pth', _use_new_zipfile_serialization=False)
...
def main():
    print("="*70)
    print("Amazon ML Challenge 2025 - BLAZING FAST Training")
    print("DistilRoBERTa: 82M params, 3x faster, 2-3 hours on P100")
    print("="*70)
    ...
    # Extract features
    print("\nExtracting features...")
    train_features = [extract_enhanced_features(row['catalog_content']) 
    ...
    # Datasets
    print("\nCreating datasets...")
    train_dataset = BlazingDataset(train_data, tokenizer, brand_enc, category_enc, unit_enc, 256, True)
    val_dataset = BlazingDataset(val_data, tokenizer, brand_enc, category_enc, unit_enc, 256, True)
    
    # Dataloaders (aggressive batch size)
    ...
    for feat in train_dataset.features_list:
        numeric = np.array([
            ...
            unit_enc.transform([feat['unit_type']])[0]
        ])
    ...
    with open('models/scaler_blazing.pkl', 'wb') as f:
        pickle.dump(scaler, f)
    with open('models/brand_encoder_blazing.pkl', 'wb') as f:
        pickle.dump(brand_enc, f)
    with open('models/category_encoder_blazing.pkl', 'wb') as f:
        pickle.dump(category_enc, f)
    with open('models/unit_encoder_blazing.pkl', 'wb') as f:
    ...
    # Train
    print("\n" + "="*70)
    print("Starting BLAZING training...")
    print("="*70)
    ...
    print("="*70)
    print("✓ Model: models/best_model_blazing.pth")
    print("✓ Use sample_code_blazing.py for inference")
    print("Expected SMAPE: 40-45% in just 2-3 hours!")
